...
export function buildConfig(answers: string[]): QuestionnaireConfig {
  const intensity = INTENSITY_MAP[answers[2]] || 5;
  return {
    answers,
    phobiaType: answers[0] || 'heights',
    intensity,
    likeType: answers[9] || 'cats',
    fearTrigger: answers[1] || 'during',
    symptom: answers[5] || 'heartbeat',
    avoidance: answers[4] || 'sometimes_avoid',
    frequency: answers[3] || 'sometimes',
    calmingStrategy: answers[8] || 'meditation',
    duration: answers[6] || 'years',
    dailyImpact: answers[7] || 'slight',
  };
}
